...
This document outlines the exhaustive list of array operations, math primitives, neural network functions, and algorithmic abstractions that `ml-switcheroo-compiler` must implement as a pure backend (providing lowerings/IR nodes). The Python frontend is now handled by `zero-jax`. These backend implementations are required to achieve 100% semantic and syntactic testing parity between `zero-optax` and the reference `optax` library.
## Core Array Creation and Manipulation (`zero_jax.numpy`)
...
## Mathematical Primitives (`zero_jax.numpy`)
...
## Reductions & Aggregations (`zero_jax.numpy`)
...
## Logical & Element-wise Operations (`zero_jax.numpy`)
...
## Neural Network Primitives (`zero_jax.nn`)
...
## Automatic Differentiation (Backend AD)
