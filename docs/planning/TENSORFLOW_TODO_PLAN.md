...
| [x] | `as_dtype` | `as_dtype(type_value) -> DType` | Converts a value to a DType object. | Used internally for type resolution passes. |
| [x] | `complex` | `complex(real: Tensor, imag: Tensor) -> Tensor` | Converts real numbers to complex. | Emits `LogicalNode` for complex instantiation. |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
| :---: | --- | --- | --- | --- |
| [x] | `Variable` | `class Variable(initial_value, trainable=True)` | A tensor whose value can be mutated. | The compiler must `lift_state` on these objects during tracing. |
| [x] | `read_variable_op` | `read_variable_op(resource: Tensor, dtype: DType) -> Tensor` | Reads the value of a variable. | Required for strict reference semantics in graph mode. |
| [x] | `assign` | `assign(ref: Variable, value: Tensor) -> Tensor` | Assigns a new value to a variable. | Must trigger state functionalization in the IR graph. |
| [x] | `assign_add` | `assign_add(ref: Variable, value: Tensor) -> Tensor` | Adds value to variable and returns. | Eager: in-place. Tracing: emits state-update node. |
| [x] | `assign_sub` | `assign_sub(ref: Variable, value: Tensor) -> Tensor` | Subtracts value from variable. | - |
| [x] | `scatter_nd_update` | `scatter_nd_update(ref: Variable, indices, updates)` | Applies sparse updates to a variable. | Requires specific IR support for sparse data structures. |
| [x] | `scatter_nd_add` | `scatter_nd_add(ref: Variable, indices, updates)` | Applies sparse addition to a variable. | - |
| [x] | `scatter_nd_sub` | `scatter_nd_sub(ref: Variable, indices, updates)` | Applies sparse subtraction to a variable. | - |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
| :---: | --- | --- | --- | --- |
| [x] | `GradientTape` | `class GradientTape(persistent=False)` | Record operations for autodiff. | Must use thread-local tape contexts inside the compiler. |
| [x] | `watch` | `watch(tensor: Tensor)` | Ensures `tensor` is traced by tape. | Required for tracking non-trainable inputs. |
| [x] | `gradient` | `gradient(target, sources) -> List[Tensor]` | Computes gradient of targets w.r.t sources. | Runs topological sort and accumulates exact VJPs. |
| [x] | `jacobian` | `jacobian(target, sources) -> List[Tensor]` | Computes jacobian of targets w.r.t sources. | Required for advanced second-order metrics/losses. |
| [x] | `batch_jacobian` | `batch_jacobian(target, sources) -> List[Tensor]` | Computes batch jacobian. | Crucial for efficient vectorized second-order derivatives. |
| [x] | `hessians` | `hessians(target, sources) -> List[Tensor]` | Computes the Hessian matrix. | Evaluates nested VJPs automatically. |
| [x] | `stop_gradient` | `stop_gradient(input: Tensor) -> Tensor` | Stops gradient from flowing backwards. | Identity in forward pass, blocks edge traversal in backward. |
| [x] | `custom_gradient` | `@custom_gradient` | Decorator for custom forward/backward passes. | Binds a python closure to a custom `LogicalNode` VJP map. |
...
| [x] | `while_loop` | `while_loop(cond, body, loop_vars)` | Repeat `body` while `cond` is true. | Emits loop/scan primitives into `ml-switcheroo-ir`. |
| [x] | `case` | `case(pred_fn_pairs, default, exclusive=False)` | Create a case operation. | Generalization of `cond`. |
| [x] | `function` | `@function(jit_compile=False)` | Compiles a function into a graph. | Sets tracing context; intercepts ops, builds `LogicalGraph`. |
| [x] | `print` | `print(*inputs, **kwargs)` | Prints inputs to standard out/err. | Requires side-effect token chaining in compiled graphs. |
| [x] | `Assert` | `Assert(condition, data, summarize=None)` | Asserts that `condition` is true. | Aborts execution statically/dynamically if False. |
...
| [x] | `fill` | `fill(dims, value, name=None) -> Tensor` | Creates a tensor filled with a scalar value. | Backs `tf.zeros` and `tf.ones`. |
...
| [x] | `size` | `size(input: Tensor) -> Tensor` | Returns the size of a tensor. | Total element count. |
| [x] | `rank` | `rank(input: Tensor) -> Tensor` | Returns the rank of a tensor. | Number of dimensions. |
| [x] | `reshape` | `reshape(tensor: Tensor, shape: Tuple) -> Tensor` | Reshapes a tensor. | Zero-copy metadata change in Eager mode. |
| [x] | `expand_dims` | `expand_dims(input: Tensor, axis: int) -> Tensor` | Inserts a dimension of 1. | - |
...
| [x] | `concat` | `concat(values: List[Tensor], axis: int) -> Tensor` | Concatenates tensors along an axis. | - |
...
| [x] | `boolean_mask` | `boolean_mask(tensor: Tensor, mask: Tensor)` | Apply boolean mask to tensor. | Equivalent to NumPy `tensor[mask]`. |
...
| [x] | `reverse` | `reverse(tensor: Tensor, axis)` | Reverses specific dimensions of a tensor. | - |
...
| [x] | `truediv` | `truediv(x: Tensor, y: Tensor) -> Tensor` | Divides x / y elementwise. | Explicit python `/` behavior. |
| [x] | `floordiv` | `floordiv(x: Tensor, y: Tensor) -> Tensor` | Divides x / y elementwise, rounding toward -inf. | Explicit python `//` behavior. |
...
| [x] | `squared_difference`| `math.squared_difference(x, y)` | Returns `(x - y)(x - y)`. | Optimization for MSE loss graphs. |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
| :---: | --- | --- | --- | --- |
| [x] | `reduce_sum` | `reduce_sum(input_tensor, axis=None, keepdims=False)` | Computes the sum of elements across dimensions. | - |
| [x] | `reduce_mean` | `reduce_mean(input_tensor, axis=None, keepdims=False)` | Computes the mean across dimensions. | Core operation for loss averages. |
| [x] | `reduce_prod` | `reduce_prod(input_tensor, axis=None, keepdims=False)` | Computes the product of elements across dimensions. | - |
| [x] | `reduce_max` | `reduce_max(input_tensor, axis=None, keepdims=False)` | Computes the maximum across dimensions. | VJP requires sparse broadcast to max index. |
| [x] | `reduce_min` | `reduce_min(input_tensor, axis=None, keepdims=False)` | Computes the minimum across dimensions. | - |
| [x] | `reduce_all` | `reduce_all(input_tensor, axis=None, keepdims=False)` | Computes logical AND across dimensions. | Output dtype is boolean. |
| [x] | `reduce_any` | `reduce_any(input_tensor, axis=None, keepdims=False)` | Computes logical OR across dimensions. | Output dtype is boolean. |
...
| [x] | `cumprod` | `math.cumprod(x, axis=0, exclusive=False, reverse=False)` | Cumulative product. | - |
...
| [x] | `cholesky` | `cholesky(input: Tensor) -> Tensor` | Computes Cholesky decomposition of square matrices. | Advanced linear algebra op. |
| [x] | `svd` | `svd(tensor: Tensor, full_matrices=False)` | Computes Singular Value Decomposition. | - |
| [x] | `qr` | `qr(input: Tensor, full_matrices=False)` | Computes QR decomposition. | - |
| [x] | `inv` | `inv(input: Tensor) -> Tensor` | Computes inverse of square matrices. | - |
| [x] | `det` | `det(input: Tensor) -> Tensor` | Computes determinant of square matrices. | - |
| [x] | `cross` | `cross(a: Tensor, b: Tensor)` | Computes pairwise cross product. | - |
| [x] | `trace` | `trace(x: Tensor)` | Computes the trace of a tensor. | Sum along diagonal. |
| [x] | `diag` | `diag(diagonal: Tensor)` | Returns a diagonal tensor with a given diagonal values. | - |
| [x] | `band_part` | `band_part(input: Tensor, num_lower, num_upper)` | Copy a tensor setting everything outside a central band to zero. | Used for causal attention masking. |
...
| [x] | `relu6` | `nn.relu6(features: Tensor) -> Tensor` | Computes Rectified Linear 6: `min(max(features, 0), 6)`. | - |
...
| [x] | `silu` | `nn.silu(features: Tensor) -> Tensor` | Computes Swish/SiLU activation. | `x * sigmoid(x)`. |
...
| [x] | `hard_sigmoid` | `nn.hard_sigmoid(x: Tensor) -> Tensor` | Piecewise linear approximation of sigmoid. | Fast inference. |
...
| [x] | `softsign` | `nn.softsign(features: Tensor) -> Tensor` | Computes softsign: `features / (abs(features) + 1)`. | - |
...
| [x] | `conv1d_transpose`| `nn.conv1d_transpose(...)` | Transposed 1D convolution. | Learns to upsample 1D sequences. |
| [x] | `conv2d_transpose`| `nn.conv2d_transpose(...)` | Transposed 2D convolution. | Learns to upsample 2D images. |
| [x] | `conv3d_transpose`| `nn.conv3d_transpose(...)` | Transposed 3D convolution. | Learns to upsample 3D volumes. |
| [x] | `depthwise_conv2d`| `nn.depthwise_conv2d(...)` | Depthwise 2-D convolution. | Separates spatial filtering from feature generation. |
| [x] | `separable_conv2d`| `nn.separable_conv2d(...)` | Separable 2-D convolution. | Depthwise followed by pointwise convolution. |
...
| [x] | `fractional_max_pool`| `nn.fractional_max_pool(...)` | Performs fractional max pooling. | Randomly generated pooling regions. |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
| :---: | --- | --- | --- | --- |
| [x] | `bias_add` | `nn.bias_add(value, bias, data_format=None)` | Adds bias to value. | Specialized `add` optimized for feature maps broadcast. |
| [x] | `batch_normalization`| `nn.batch_normalization(x, mean, var, offset, scale, eps)`| Batch normalization. | Tricky state handling. The compiler must intercept and route running mean/var properly. |
| [x] | `layer_norm` | `nn.layer_norm(x, scale, offset)` | Layer normalization. | Independently normalizes the last dimension. |
| [x] | `l2_normalize` | `math.l2_normalize(x, axis=None, epsilon=1e-12)` | Normalizes along dimension using L2 norm. | - |
...
| [x] | `top_k` | `math.top_k(input, k=1, sorted=True)` | Finds values and indices of the `k` largest entries. | Required for decoding. |
| [x] | `in_top_k` | `math.in_top_k(targets, predictions, k)` | Says whether the targets are in the top `K` predictions. | Used for SparseTopK metrics. |
| [x] | `one_hot` | `one_hot(indices, depth, on_value=1, off_value=0)`| Returns a one-hot tensor. | Converts integer sparse classes to dense distributions. |
| [x] | `ctc_loss` | `nn.ctc_loss(labels, logits, label_length, logit_length)`| Computes the CTC (Connectionist Temporal Classification) Loss. | Specialized sequence-to-sequence loss mapping. |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
| :---: | --- | --- | --- | --- |
| [x] | `random_normal` | `random.normal(shape, mean=0.0, stddev=1.0)` | Random values from a normal distribution. | Maps to stateful or seeded compiler generator. |
| [x] | `random_uniform` | `random.uniform(shape, minval=0, maxval=None)`| Random values from a uniform distribution. | - |
| [x] | `truncated_normal`| `random.truncated_normal(shape, mean, stddev)` | Outputs random values from a truncated normal distribution. | Prevents >2 stddev outliers. |
| [x] | `categorical` | `random.categorical(logits, num_samples)` | Draws samples from a categorical distribution. | Necessary for discrete action sampling in RL. |
| [x] | `poisson` | `random.poisson(shape, lam)` | Draws samples from a Poisson distribution. | - |
| [x] | `gamma` | `random.gamma(shape, alpha)` | Draws samples from a Gamma distribution. | - |
| [x] | `set_seed` | `random.set_seed(seed: int)` | Sets the global graph random seed. | Must guarantee fully deterministic `zero-zoo` headless validation. |
| [x] | `stateless_random_normal`| `random.stateless_random_normal(shape, seed)` | Stateless random normal. | Pure functional RNG, requires strict PRNG key threading. |
| [x] | `stateless_random_uniform`| `random.stateless_random_uniform(shape, seed)` | Stateless random uniform. | Pure functional RNG. |
...
| [x] | `TensorArray` | `class TensorArray(dtype, size, dynamic_size)` | Class wrapping dynamic-sized, per-time-step, write-once Tensor arrays. | Used natively inside RNNs and `while_loop` tracing bounds. |
