...
| :---: | :--- | :--- | :--- | :--- |
| [ ] | `Module` | `class Module:` | Base class for all neural network modules in NNX. | Compiler must support extracting dynamic `__dict__` state into tracing context and re-injecting variables post-execution. |
...
| [ ] | `State` | `class State(dict[str, Any]):` | A nested dictionary structure for `nnx` variables. | Compiler must support structural traversal, updates, and PyTree flattening of this custom dictionary type. |
...
| [ ] | `Linear` | `Linear(in_features: int, out_features: int, use_bias: bool)` | Alias/variant of standard linear (dense) layer. | Same structural compiler requirements as `Dense`. |
...
| [ ] | `Einsum` | `Einsum(einsum_str: str, kernel_shape: tuple)` | A module that performs a linear transformation using an einsum equation. | Compiler must fully support Einstein summation notation parsing and execution. |
...
| Status | Name | Class Signature | Docstring | Compiler Support Notes |
| :---: | :--- | :--- | :--- | :--- |
| [ ] | `Conv` | `Conv(in_features: int, out_features: int, kernel_size: tuple, strides: tuple, padding: str\|tuple)` | A general n-dimensional convolutional layer. | The compiler must accurately map this to `conv_general_dilated` taking into account arbitrary NCHW/NHWC data formats. |
...
| Status | Name | Class Signature | Docstring | Compiler Support Notes |
| :---: | :--- | :--- | :--- | :--- |
| [ ] | `BatchNorm` | `BatchNorm(num_features: int, use_running_average: bool)` | Batch Normalization layer. | **Crucial:** Compiler must intercept the running `mean`/`var` mutations and push the updated `BatchStat` back to the Python object. |
| [ ] | `LayerNorm` | `LayerNorm(num_features: int, reduction_axes: int)` | Layer Normalization layer. | Verifies `mean` and `var` reductions over specific spatial/channel dimensions. |
...
| Status | Name | Class Signature | Docstring | Compiler Support Notes |
| :---: | :--- | :--- | :--- | :--- |
| [ ] | `Dropout` | `Dropout(rate: float, rng_collection: str)` | A dropout layer (randomly zeroing elements). | Compiler must consume `rng_collection` (usually 'dropout'), split the key, and trace the bernoulli mask. |
...
| [ ] | `Vmap` | `Vmap(module_constructor: Callable)` | Vectorizes the execution of a sub-module. | Needs to lift the state parameters across the batch dimension. |
| [ ] | `Scan` | `Scan(module_constructor: Callable)` | Loops over a sequence of inputs, maintaining module state. | Essential for RNNs. Compiler must manage tied parameters while evolving `BatchStat` or `Rng` states. |
...
| [ ] | `Pmap` | `Pmap(module_constructor: Callable)` | Parallelizes module execution across multiple devices. | State needs to be sharded or broadcast appropriately. |
