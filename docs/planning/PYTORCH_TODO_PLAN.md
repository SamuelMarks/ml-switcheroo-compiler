...
| [x] | `ShapeInference` | `def infer_shape(a_shape, b_shape)` | Numpy-compatible static shape inference for complex broadcast logic | Crucial for validation pre-compile |
...
| [x] | `erfinv` | `(*args, **kwargs)` | Applies the erfinv operation. | Map to IR Node |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
|---|---|---|---|---|
| [x] | `cholesky` | `(*args, **kwargs)` | Applies the cholesky operation. | Map to IR Node |
| [x] | `det` | `(*args, **kwargs)` | Applies the det operation. | Map to IR Node |
| [x] | `diag` | `(*args, **kwargs)` | Applies the diag operation. | Map to IR Node |
| [x] | `dot` | `(*args, **kwargs)` | Applies the dot operation. | Map to IR Node |
| [x] | `eigh` | `(*args, **kwargs)` | Applies the eigh operation. | Map to IR Node |
...
| [x] | `inv` | `(*args, **kwargs)` | Applies the inv operation. | Map to IR Node |
...
| [x] | `pinv` | `(*args, **kwargs)` | Applies the pinv operation. | Map to IR Node |
| [x] | `qr` | `(*args, **kwargs)` | Applies the qr operation. | Map to IR Node |
| [x] | `slogdet` | `(*args, **kwargs)` | Applies the slogdet operation. | Map to IR Node |
| [x] | `svd` | `(*args, **kwargs)` | Applies the svd operation. | Map to IR Node |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
|---|---|---|---|---|
| [x] | `TopologicalSort` | `def topsort(node)` | Topological Graph Sorting algorithm | Dependency resolution for backprop |
| [x] | `VJPs` | `def vjp(node, grad_out)` | Defines exact mathematical VJPs for all Operations | Jacobian-vector products |
| [x] | `GradientAccumulation` | `def accumulate_grads(grads)` | Gradient Accumulation routines for nodes with multiple dependents | Resolves fan-out scenarios |
| [x] | `AutogradMatching` | `def verify_autograd()` | Provide strict tensor equality checks and chaining rules | Ensures match with `zero-pytorch.autograd` |

## 4. Tensor State & Autograd Utilities

| Checkbox | Name | Function/Class Signature | Docstring | Notes |
|---|---|---|---|---|
| [x] | `backward` | `Tensor.backward(*args, **kwargs)` | Triggers the reverse-mode auto-differentiation | Maps to `compiler.grad` |
| [x] | `view` | `Tensor.view(*shape)` | Returns a new tensor with the same data but different size | Aliased shape logic |
| [x] | `contiguous` | `Tensor.contiguous()` | Returns a contiguous in memory tensor | Ensures memory layout |
| [x] | `item` | `Tensor.item()` | Returns the value of this tensor as a standard Python number | Forces eager evaluation |
| [ ] | `detach` | `Tensor.detach()` | Returns a new Tensor, detached from the current graph | Stops tracing/gradient flow |
| [x] | `no_grad` | `class no_grad()` | Context manager that disabled gradient calculation | Disables `TracerTape` recording |
| [x] | `set_grad_enabled` | `class set_grad_enabled(mode)` | Context manager for gradient calculation | Controls `TracerTape` state |
...
| [x] | `adaptive_avg_pool1d` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_avg_pool1d` equivalent | Lowered from module |
| [x] | `adaptive_avg_pool2d` | `(*args, **kwargs)` | Applies the adaptive_avg_pool2d operation. | Lowered from module |
| [x] | `adaptive_avg_pool3d` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_avg_pool3d` equivalent | Lowered from module |
| [x] | `adaptive_max_pool1d` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_max_pool1d` equivalent | Lowered from module |
| [x] | `adaptive_max_pool2d` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_max_pool2d` equivalent | Lowered from module |
| [x] | `adaptive_max_pool3d` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_max_pool3d` equivalent | Lowered from module |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
|---|---|---|---|---|
| [x] | `adaptive_log_softmax_with_loss` | `(*args, **kwargs)` | PyTorch `nn.functional.adaptive_log_softmax_with_loss` equivalent | Lowered from module |
...
| Checkbox | Name | Function/Class Signature | Docstring | Notes |
|---|---|---|---|---|
| [x] | `complex` | `(*args, **kwargs)` | Applies the complex operation. | Lowered from module |
...
| [x] | `OptimizerSupport` | `def lift_optimizer_state(graph)` | Traces optimizer state updates (momentum, velocity, steps) | Stateful updates for Optimizers |
| [x] | `DCE` | `def dead_code_elimination(graph)` | Prune unreferenced operations | Memory / compute optimization |
| [x] | `CSE` | `def common_subexpression_elimination(graph)` | De-duplicate identically computed nodes | Prevents redundant ops |
| [x] | `IR_Translation` | `def to_logical_graph(tape)` | Output cleanly to `ml-switcheroo-ir` compliant `LogicalGraph` | Must tie back to user AST refs |
