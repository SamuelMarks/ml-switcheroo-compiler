...
def test_prepare_depthwise_conv_extra() -> object:
    ...
    t1 = Tensor(np.ones((1, 5, 2)), TensorConfig((1, 5, 2), "float32", device))  # batch, spatial, channels
    t2 = Tensor(np.ones((3, 2, 4)), TensorConfig((3, 2, 4), "float32", device))  # spatial, in_channels, channel_multiplier
