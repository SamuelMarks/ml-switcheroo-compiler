...
def test_group_mean_eager():
    ...
    out = execute_generic_op(np, "GroupMean", x, 2, axis=-1)
...
def test_group_variance_eager():
    x = np.random.randn(2, 4, 4, 8).astype(np.float32)
    out = execute_generic_op(np, "GroupVariance", x, 2, axis=-1)
...
def test_group_norm_eager():
    x = np.random.randn(2, 4, 4, 8).astype(np.float32)
    out = execute_generic_op(np, "GroupNorm", x, 2, axis=-1, epsilon=1e-5)
...
def test_group_norm_5d_eager():
    ...
    out = execute_generic_op(
        np, "GroupNorm", x, groups=4, weight=weight, bias=bias, axis=-1, epsilon=1e-5
    )
