import numpy as np
from ml_switcheroo_compiler.ops.linalg.decompositions import Tridiagonal, polar
...
def test_tridiagonal_infer_shape():
    device = Device("cpu")
...
def test_polar_tracing():
    device = Device("cpu")
    t1 = Tensor(np.ones((2, 2)), TensorConfig((2, 2), "float32", device))
    with ConfigContext(eager_mode=False):
        with patch(
            "ml_switcheroo_compiler.ops.linalg.decompositions._emit_linalg_node"
        ) as mock_emit:
            polar(t1)
