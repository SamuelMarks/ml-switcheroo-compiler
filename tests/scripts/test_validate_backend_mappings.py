...
def test_validate_mappings():
    with tempfile.TemporaryDirectory() as d:
        ...
        mock_yaml = {
            "backend_name": "torch",
            "operations": {
                "valid_op": {"target_api": "torch.add"},
                "valid_op_kwargs": {"target_api": "torch.sum", "kwarg_translations": {"dim": "axis", "keepdim": {"target_name": "keepdims"}, "unmapped": {"target_name": "unmapped_kwarg"}}},
                "invalid_op": {"target_api": "torch.hallucinated_op"},
                "custom_op": {"target_api": "custom_op"},
                "ast_template_op": {"ast_template": "torch.sub(a, b)"},
                "ast_template_invalid": {"ast_template": "torch.hallucinated_sub(a, b)"},
                "ast_template_no_match": {"ast_template": "!invalid"},
                "lambda_op": {"target_api": "lambda x: x"},
                "ignored_op": {"target_api": "torch.abs"},  # explicitly ignored in script
        ...
        mock_api_dict = {"add": set(), "sum": {"axis", "keepdims"}, "sub": set()}
        ...
        assert "torch.abs" not in err_str
