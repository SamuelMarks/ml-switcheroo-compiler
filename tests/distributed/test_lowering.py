...
def test_torch_spmd_lowering():
    ...
    code_gather = gen.visit(node_gather, ["x"])
    assert (
        "torch.distributed.all_gather_into_tensor" in code_gather
        or "torch.distributed.all_gather" in code_gather
        or "dist.all_gather" in code_gather
    )

    node_scatter = IRNode(id="n2", op_type="reduce_scatter", inputs=["x"])
    code_scatter = gen.visit(node_scatter, ["x"])
    assert (
        "torch.distributed.reduce_scatter_tensor" in code_scatter
        or "torch.distributed.reduce_scatter" in code_scatter
        or "dist.reduce_scatter" in code_scatter
    )
