"""Unit tests for TCP distributed context lifecycle and collective operations."""
...
def test_axis_index() -> None:
    """Verify axis index resolution for distributed backend."""
    assert _np_axis_index(np) == 0


def test_tcp_dist_ctx_init() -> None:
    """Verify initialization of TCPDistributedContext with missing config."""
    with patch("os.path.exists", return_value=False):
        ctx = TCPDistributedContext(topology="tree")
        assert ctx.config == {}


def test_tcp_dist_ctx_initialize_tree() -> None:
    """Verify TCPDistributedContext connection establishment across tree topologies."""
    with patch("os.path.exists", return_value=False):
...
def test_tcp_dist_ctx_all_reduce_ops() -> None:
    """Verify ring all-reduce operations and gather behaviors."""
...
def test_tcp_dist_ctx_shutdown() -> None:
    """Verify graceful resource cleanup during TCPDistributedContext shutdown."""
...
def test_tcp_distributed_ops_dispatch() -> None:
    """Verify dispatching of distributed ops with active TCPDistributedContext."""
    ...
    with patch(
        "ml_switcheroo_compiler.backends.numpy.eager.distributed.TCPDistributedContext.all_gather_tensors",
        return_value=[np.array([1.0]), np.array([2.0])],
    ):
        with patch(
            "ml_switcheroo_compiler.backends.numpy.eager.distributed.TCPDistributedContext.all_reduce_ring",
