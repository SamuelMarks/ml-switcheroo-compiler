...
import numpy as np
from ml_switcheroo_compiler.chex import (
...
def test_assert_axis_dimension() -> None:
    ...
    # Dynamic shape
    t_dyn = Tensor(np.zeros((2, 3)), shape=(2, "dim1"), dtype=DType.Float32, device="cpu")
    assert_axis_dimension(t_dyn, 1, 3)
...
def test_assert_trees_all_close() -> None:
    ...
    config.eager_mode = True
    t1 = Tensor(np.array([1.0, 2.0]), shape=(2,), dtype=DType.Float32, device="cpu")
    t2 = Tensor(np.array([1.0, 2.0000001]), shape=(2,), dtype=DType.Float32, device="cpu")
...
def test_chex_missing_branches() -> None:
    ...
    from ml_switcheroo_compiler.chex import warn_only_n_pos_args_in_future
    ...
    from ml_switcheroo_compiler.chex import all_variants, variants, warn_keyword_args_only_in_future
...
def test_chexify_call() -> None:
    """Test chexify call."""
    from ml_switcheroo_compiler.chex import chexify
