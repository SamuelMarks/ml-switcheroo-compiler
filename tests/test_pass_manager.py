...
def test_pass_manager_run_fixpoint() -> None:
    """Docstring."""
    pm = PassManager()
    ...
    pm.add_pass(dummy_pass)
    pm.run_fixpoint(graph, max_iterations=5)

    assert counter == 2


def test_pass_manager_run_fixpoint_max_iters() -> None:
    """Docstring."""
    pm = PassManager()
    ...
    pm.add_pass(dummy_pass)
    pm.run_fixpoint(graph, max_iterations=3)
