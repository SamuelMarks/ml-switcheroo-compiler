...
def zeros_like(
    ...
    """Returns a tensor filled with scalar 0, with the same size as `input`."""
...
def ones_like(
    ...
    """Returns a tensor filled with scalar 1, with the same size as `input`."""
    dtype = dtype or input.dtype
    device = device or input.device
    if config.eager_mode:
...
def linspace(
    ...
    """Creates a 1D tensor evenly spaced from `start` to `stop`."""
    dtype = dtype or config.default_float_dtype
    device = device or config.default_device
    shape = (steps,)
...
def diag(input: Tensor, diagonal: int = 0) -> Tensor:
    """Returns a 2-D square tensor with diagonal, or extracts diagonal."""
    device = input.device
    dtype = input.dtype
