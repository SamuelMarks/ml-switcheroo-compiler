...
def shape_aware_rewrite(graph: LogicalGraph) -> LogicalGraph:
    """Resolves structural ambiguities and enforces strict typing using inferred shapes.
    ...
    new_graph = LogicalGraph(
        name=f"{graph.name}_rewritten", outputs=list(graph.outputs), mesh=graph.mesh
    )
    ...
    for node in topological_sort(graph):
        ...
        if node.op_type == "Reshape":
            if node.shape_metadata is not None:
