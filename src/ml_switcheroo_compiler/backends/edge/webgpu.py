...
class WebGPUCodeGenerator(BaseGenerator):
    ...
    def _get_wgsl_for_op(self, node: IRNode, shape: list[int], nelem: int, clean_id: str) -> tuple[list[str], str, str, str]:
        ...
        if template.get("body"):
            formatted_body: str = template["body"].format(**expr_format_args)
        ...
        func: WGSLFunction = WGSLFunction(f"compute_{clean_id}", ["@builtin(global_invocation_id) global_id: vec3<u32>", "@builtin(local_invocation_id) local_id: vec3<u32>"], body_nodes, [f"@compute @workgroup_size({wg_x}, {wg_y}, {wg_z})"])

        if mapping["template"] == "matmul":
            dispatch_x: str = f"Math.ceil({expr_format_args['N']} / {wg_x})"
            dispatch_y: str = f"Math.ceil({expr_format_args['M']} / {wg_y})"
            dispatch_z: str = "1"
            ...
            dispatch_x: str = f"Math.ceil({expr_format_args.get('out_width', 1)} / {wg_x})"
            dispatch_y: str = f"Math.ceil({expr_format_args.get('out_height', 1)} / {wg_y})"
            dispatch_z: str = "1"
        else:
            wg_total: int = wg_x * wg_y * wg_z
            dispatch_x: str = f"Math.ceil({nelem} / {wg_total})"
            dispatch_y: str = "1"
            dispatch_z: str = "1"
    ...
    def visit_AllReduce(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_AllGather(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_AllToAll(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_ReduceScatter(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_WhileLoop(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_Cond(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
    ...
    def visit_Scan(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
        ...
        for node in self.sorted_nodes:
            ...
            if hasattr(self, f"visit_{op_type}"):
                ...
                try:
                    ...
                    op_wgsl: list[str] = ["@compute @workgroup_size(64, 1, 1)", f"fn compute_fallback_{clean_id}(@builtin(global_invocation_id) global_id : vec3<u32>) {{", "    // Missing shader implementation fallback", "}"]
                    dispatch_x: str = str((nelem + 63) // 64)
                    dispatch_y: str = "1"
                    dispatch_z: str = "1"
            wgsl.extend(op_wgsl)
    ...
    def visit_Conv2D(self, node: IRNode, input_vars: list[str], **kwargs: Any) -> tuple[list[str], str, str, str]:
