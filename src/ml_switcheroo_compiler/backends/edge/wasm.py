...
class WasmCodeGenerator(BaseGenerator):
    ...
    def generic_visit(self, node: object, input_vars: list[str], **kwargs: object) -> str:
        ...
        if self.is_simd:
            ...
            if op_type == "Constant":
                ...
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_splat({val});")
                return res_var

            in_vars_mapped = [self.var_map.get(inp, inp) for inp in getattr(node, "inputs", [])]

            if op_type == "Add":
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_add({in_vars_mapped[0]}, {in_vars_mapped[1]});")
            elif op_type == "Subtract":
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_sub({in_vars_mapped[0]}, {in_vars_mapped[1]});")
            elif op_type == "Multiply":
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_mul({in_vars_mapped[0]}, {in_vars_mapped[1]});")
            elif op_type in ("TrueDivide", "Div"):
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_div({in_vars_mapped[0]}, {in_vars_mapped[1]});")
            elif op_type in ("Exp", "Log"):
                func = "std::exp" if op_type == "Exp" else "std::log"
                extracts = [f"{func}(wasm_f32x4_extract_lane({in_vars_mapped[0]}, {i}))" for i in range(4)]
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_make({', '.join(extracts)});")
            elif op_type in ("Negative", "Neg"):
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_neg({in_vars_mapped[0]});")
            else:
                extracts = [f"{op_type.lower()}(wasm_f32x4_extract_lane({in_vars_mapped[0]}, {i}))" for i in range(4)]
                self.body_lines_simd.append(f"    v128_t {res_var} = wasm_f32x4_make({', '.join(extracts)});")
            ...
            if op_type == "Constant":
                ...
                self.body_lines_scalar.append(f"    {dtype_c} {res_var} = {val};")
                return res_var
            ...
            if op_type in op_map:
                ...
                self.body_lines_scalar.append(f"    {dtype_c} {res_var} = {expr};")
            elif op_type in ("Exp", "Log"):
                func_name = f"std::{op_type.lower()}"
                self.body_lines_scalar.append(f"    {dtype_c} {res_var} = {func_name}({in_vars_mapped[0]});")
            elif op_type in ("Negative", "Neg"):
                self.body_lines_scalar.append(f"    {dtype_c} {res_var} = -{in_vars_mapped[0]};")
            else:
                args_str = ", ".join(in_vars_mapped)
                self.body_lines_scalar.append(f"    {dtype_c} {res_var} = {op_type.lower()}({args_str});")
    ...
    def generate(self) -> str:
        ...
        # Dual structures to track variable mappings during two visits
        self.var_map_simd: dict[str, str] = {}
        self.var_map_scalar: dict[str, str] = {}

        # Prepare parameters for the function signature
        func_params = []
        ...
        # Build C++ kernel wrapper headers
        ...
        # 1. Generate WASM SIMD Loop Block
        self.is_simd = True
        self.var_map = self.var_map_simd
        # Pre-populate and load input variables for SIMD loop
        for idx, node in enumerate(input_nodes):
            nid = getattr(node, "id", "")
        ...
        # Emit output assignments in Scalar remainder loop
        for i, out_id in enumerate(output_ids):
