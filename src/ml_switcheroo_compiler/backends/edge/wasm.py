...
class WasmCodeGenerator(BaseGenerator):
    ...
    def _generate_fallback_copy(self, clean_id: str, inputs: list[str], nelem: int) -> None:
        self.add_line("  // SIMD Fallback Copy")
    ...
    def _generate_conv(self, node: Any, op_type: str, clean_id: str, inputs: list[str], shape: list[int], nelem: int) -> None:
        self.add_line(f"  // Naive {op_type} implementation")
        in0_node = next((n for n in self.sorted_nodes if getattr(n, "id", "").replace("-", "_") == inputs[0]), None)
        in1_node = next((n for n in self.sorted_nodes if getattr(n, "id", "").replace("-", "_") == inputs[1]), None) if len(inputs) > 1 else None

        in0_shape = getattr(in0_node, "shape_metadata", None) if in0_node else [1, 1, 1, 1]
        if not in0_shape:
            in0_shape = [1, 1, 1, 1]
        elif isinstance(in0_shape, (int, float)):
            in0_shape = [int(in0_shape)]

        in1_shape = getattr(in1_node, "shape_metadata", None) if in1_node else [1, 1, 1, 1]
        if not in1_shape:
            in1_shape = [1, 1, 1, 1]
        elif isinstance(in1_shape, (int, float)):
            in1_shape = [int(in1_shape)]

        if len(in0_shape) == 4 and len(in1_shape) == 4 and len(shape) == 4:
            # Assume NCHW for input/output and OIHW for weight
            N, C, H, W = in0_shape
            outC, inC, kH, kW = in1_shape
            _, _, outH, outW = shape
            ...
            if hasattr(node, "attributes"):
                strides = node.attributes.get("window_strides", [1, 1])
                padding = node.attributes.get("padding", [[0, 0], [0, 0]])
                ...
                if isinstance(padding, (list, tuple)) and len(padding) == 2:
                    if isinstance(padding[0], (list, tuple)):
                        ...
                        pad_h = padding[0]
                    if isinstance(padding[1], (list, tuple)):
                        pad_w = padding[1][0]
                    else:
                        pad_w = padding[1]

            self.add_line(f"  for (int n = 0; n < {N}; ++n) {{")
            self.add_line(f"    for (int o = 0; o < {outC}; ++o) {{")
            self.add_line(f"      for (int oh = 0; oh < {outH}; ++oh) {{")
            self.add_line(f"        for (int ow = 0; ow < {outW}; ++ow) {{")
            self.add_line("          float sum = 0.0f;")
            self.add_line(f"          for (int c = 0; c < {C}; ++c) {{")
            self.add_line(f"            for (int kh = 0; kh < {kH}; ++kh) {{")
            self.add_line(f"              for (int kw = 0; kw < {kW}; ++kw) {{")
            self.add_line(f"                int ih = oh * {stride_h} - {pad_h} + kh;")
            self.add_line(f"                int iw = ow * {stride_w} - {pad_w} + kw;")
            self.add_line(f"                if (ih >= 0 && ih < {H} && iw >= 0 && iw < {W}) {{")
            self.add_line(f"                  int in_idx = n * ({C} * {H} * {W}) + c * ({H} * {W}) + ih * {W} + iw;")
            self.add_line(f"                  int w_idx = o * ({inC} * {kH} * {kW}) + c * ({kH} * {kW}) + kh * {kW} + kw;")
            self.add_line(f"                  sum += buf_{inputs[0]}[in_idx] * buf_{inputs[1]}[w_idx];")
            self.add_line("                }")
            self.add_line("              }")
            self.add_line("            }")
            self.add_line("          }")
            self.add_line(f"          int out_idx = n * ({outC} * {outH} * {outW}) + o * ({outH} * {outW}) + oh * {outW} + ow;")
            self.add_line(f"          buf_{clean_id}[out_idx] = sum;")
            self.add_line("        }")
            self.add_line("      }")
            self.add_line("    }")
            self.add_line("  }")
        else:
            self._generate_fallback_copy(clean_id, inputs, nelem)

    def _generate_pool(self, node: Any, op_type: str, clean_id: str, inputs: list[str], shape: list[int], nelem: int) -> None:
        self.add_line(f"  // Naive {op_type} implementation")
        in0_node = next((n for n in self.sorted_nodes if getattr(n, "id", "").replace("-", "_") == inputs[0]), None)
        in0_shape = getattr(in0_node, "shape_metadata", None) if in0_node else [1, 1, 1, 1]
        if not in0_shape:
        ...
        if len(in0_shape) == 4 and len(shape) == 4:
            N, C, H, W = in0_shape
            _, _, outH, outW = shape
            ...
            pad_h, pad_w = 0, 0
            if hasattr(node, "attributes"):
                window_dims = node.attributes.get("window_dimensions", [2, 2])
                if len(window_dims) >= 2:
                    kH, kW = window_dims[-2], window_dims[-1]
                strides = node.attributes.get("window_strides", [2, 2])
                if len(strides) >= 2:
                    stride_h, stride_w = strides[-2], strides[-1]
                padding = node.attributes.get("padding", [[0, 0], [0, 0]])
                if isinstance(padding, (list, tuple)) and len(padding) >= 2:
                    p = padding[-2:]
            ...
            self.add_line(f"        for (int ow = 0; ow < {outW}; ++ow) {{")
            if is_max:
                self.add_line("          float best = -INFINITY;")
            else:
                self.add_line("          float sum = 0.0f;")
                self.add_line("          int count = 0;")
            self.add_line(f"          for (int kh = 0; kh < {kH}; ++kh) {{")
            self.add_line(f"            for (int kw = 0; kw < {kW}; ++kw) {{")
            self.add_line(f"              int ih = oh * {stride_h} - {pad_h} + kh;")
            self.add_line(f"              int iw = ow * {stride_w} - {pad_w} + kw;")
            self.add_line(f"              if (ih >= 0 && ih < {H} && iw >= 0 && iw < {W}) {{")
            self.add_line(f"                int in_idx = n * ({C} * {H} * {W}) + c * ({H} * {W}) + ih * {W} + iw;")
            self.add_line(f"                float val = buf_{inputs[0]}[in_idx];")
            if is_max:
                self.add_line("                if (val > best) best = val;")
            else:
                self.add_line("                sum += val;")
                self.add_line("                count++;")
            self.add_line("              }")
            self.add_line("            }")
            self.add_line("          }")
            self.add_line(f"          int out_idx = n * ({C} * {outH} * {outW}) + c * ({outH} * {outW}) + oh * {outW} + ow;")
            if is_max:
                self.add_line(f"          buf_{clean_id}[out_idx] = best;")
            else:
                self.add_line(f"          buf_{clean_id}[out_idx] = count > 0 ? sum / count : 0.0f;")
            self.add_line("        }")
            self.add_line("      }")
            self.add_line("    }")
            self.add_line("  }")
    ...
    def _generate_norm(self, node: Any, op_type: str, clean_id: str, inputs: list[str], shape: list[int], nelem: int) -> None:
        self.add_line(f"  // Naive {op_type} implementation")
        in0_node = next((n for n in self.sorted_nodes if getattr(n, "id", "").replace("-", "_") == inputs[0]), None)
        ...
        # Batch norm usually operates on the C dimension for NCHW
        # Layer norm usually operates on the last D dimensions
        # For simplicity, we implement a global layer norm fallback if specific parameters aren't found
        if op_type == "BatchNorm" and len(in0_shape) == 4 and len(inputs) >= 5:
            ...
            self.add_line(f"  for (int n = 0; n < {N}; ++n) {{")
            self.add_line(f"    for (int c = 0; c < {C}; ++c) {{")
            self.add_line(f"      float mean = buf_{inputs[3]}[c];")
            self.add_line(f"      float var = buf_{inputs[4]}[c];")
            self.add_line(f"      float weight = buf_{inputs[1]}[c];")
            self.add_line(f"      float bias = buf_{inputs[2]}[c];")
            self.add_line(f"      for (int h = 0; h < {H}; ++h) {{")
            self.add_line(f"        for (int w = 0; w < {W}; ++w) {{")
            self.add_line(f"          int idx = n * ({C} * {H} * {W}) + c * ({H} * {W}) + h * {W} + w;")
            self.add_line(f"          float norm_val = (buf_{inputs[0]}[idx] - mean) / std::sqrt(var + {eps}f);")
            self.add_line(f"          buf_{clean_id}[idx] = norm_val * weight + bias;")
            self.add_line("        }")
            self.add_line("      }")
            self.add_line("    }")
            self.add_line("  }")
        elif op_type == "LayerNorm" and len(inputs) >= 3:
            # Naive layer norm over the last dimension
            outer = 1
            for s in in0_shape[:-1]:
                outer *= s
            inner = in0_shape[-1]
            eps = 1e-5
            if hasattr(node, "attributes"):
                eps = node.attributes.get("epsilon", 1e-5)
            self.add_line(f"  for (int o = 0; o < {outer}; ++o) {{")
            self.add_line("    float sum = 0.0f;")
            self.add_line(f"    for (int i = 0; i < {inner}; ++i) {{")
            self.add_line(f"      sum += buf_{inputs[0]}[o * {inner} + i];")
            self.add_line("    }")
            self.add_line(f"    float mean = sum / {inner}.0f;")
            self.add_line("    float var_sum = 0.0f;")
            self.add_line(f"    for (int i = 0; i < {inner}; ++i) {{")
            self.add_line(f"      float diff = buf_{inputs[0]}[o * {inner} + i] - mean;")
            self.add_line("      var_sum += diff * diff;")
            self.add_line("    }")
            self.add_line(f"    float var = var_sum / {inner}.0f;")
            self.add_line(f"    for (int i = 0; i < {inner}; ++i) {{")
            self.add_line(f"      float norm_val = (buf_{inputs[0]}[o * {inner} + i] - mean) / std::sqrt(var + {eps}f);")
            self.add_line(f"      buf_{clean_id}[o * {inner} + i] = norm_val * buf_{inputs[1]}[i] + buf_{inputs[2]}[i];")
            self.add_line("    }")
            self.add_line("  }")
        else:
            self._generate_fallback_copy(clean_id, inputs, nelem)
    ...
    def _generate_op(self, node: Any, op_type: str, clean_id: str, inputs: list[str], shape: list[int], nelem: int) -> None:
        ...
        elif op_type in ("Conv1D", "Conv2D", "Conv3D", "ConvTranspose2D", "ConvGeneralDilated"):
            self._generate_conv(node, op_type, clean_id, inputs, shape, nelem)
        elif op_type in ("MaxPool", "AvgPool", "MaxPool2D", "AvgPool2D"):
            self._generate_pool(node, op_type, clean_id, inputs, shape, nelem)
        elif op_type in ("BatchNorm", "LayerNorm", "GroupNorm"):
            self._generate_norm(node, op_type, clean_id, inputs, shape, nelem)
        elif op_type in ("Reshape", "Transpose", "Concat", "Slice", "Gather", "Scatter"):
            self._generate_fallback_copy(clean_id, inputs, nelem)
