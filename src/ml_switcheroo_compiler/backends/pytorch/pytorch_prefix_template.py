...
PT_PREFIX_TEMPLATE = """def pt_mel_filterbank(num_mel_bins, num_spectrogram_bins, sample_rate, lower_edge_hertz, upper_edge_hertz):
    import torchaudio
...
def pt_mfcc(spectrogram, sample_rate, num_mel_bins, lower_edge_hertz, upper_edge_hertz, num_mfccs):
    ...
    return torchaudio.functional.create_dct(num_mfccs, num_mel_bins, 'ortho').to(spectrogram.device).matmul(log_mel.unsqueeze(-2)).squeeze(-2)
