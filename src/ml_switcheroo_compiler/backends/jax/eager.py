...
def _execute_accumulate_n(*args: Any, **kwargs: Any) -> Any:
    ...
    """
    inputs: Any = args[0] if len(args) > 0 else kwargs.get("inputs", [])
    if not inputs:
...
def _execute_cast(*args: Any, **kwargs: Any) -> Any:
    ...
    """
    tensor: Any = args[0]
    dtype: Any = kwargs.get("dtype") if "dtype" in kwargs else args[1]
...
def _execute_ragged_tensor_to_dense(*args: Any, **kwargs: Any) -> Any:
    ...
    if isinstance(rt, (list, tuple)) and len(rt) > 0 and hasattr(rt[0], "shape"):
        ...
        padded: list[Any] = [jnp.pad(x, (0, max_len - len(x))) for x in rt]
...
_OP_DISPATCH = {
    ...
    "AdaptiveLogSoftmaxWithLoss": lambda input, target, *args, **kwargs: (target, jax.numpy.zeros((), dtype=getattr(target, "dtype", None))),
    ...
    "Atleast1d": jax.numpy.atleast_1d,
    "Atleast2d": jax.numpy.atleast_2d,
    "Atleast3d": jax.numpy.atleast_3d,
    "AxisIndex": lambda *args, **kwargs: jax.numpy.array(0),
    "BesselI0": jax.scipy.special.i0,
    "BesselI0e": jax.scipy.special.i0e,
    "BesselI1": jax.scipy.special.i1,
    "BesselI1e": jax.scipy.special.i1e,
    "Frombuffer": lambda *args, **kwargs: jax.numpy.frombuffer(args[0], **kwargs),
    "Fft2": lambda *args, **kwargs: jax.numpy.fft.fft2(args[0], **kwargs),
    "Fftfreq": lambda *args, **kwargs: jax.numpy.fft.fftfreq(*args, **kwargs),
    "Fftn": lambda *args, **kwargs: jax.numpy.fft.fftn(args[0], **kwargs),
    "Fftnd": lambda *args, **kwargs: jax.numpy.fft.fftn(args[0], **kwargs),
    "Fftshift": lambda *args, **kwargs: jax.numpy.fft.fftshift(args[0], **kwargs),
    "HardSilu": lambda x: jax.nn.hard_silu(x),
    "HardSwish": lambda x: jax.nn.hard_swish(x),
    "Hfft": lambda *args, **kwargs: jax.numpy.fft.hfft(args[0], **kwargs),
    "Ifft": lambda *args, **kwargs: jax.numpy.fft.ifft(args[0], **kwargs),
    "Ifft2": lambda *args, **kwargs: jax.numpy.fft.ifft2(args[0], **kwargs),
    "Ifftn": lambda *args, **kwargs: jax.numpy.fft.ifftn(args[0], **kwargs),
    "Ifftnd": lambda *args, **kwargs: jax.numpy.fft.ifftn(args[0], **kwargs),
    "Ifftshift": lambda *args, **kwargs: jax.numpy.fft.ifftshift(args[0], **kwargs),
    "Ihfft": lambda *args, **kwargs: jax.numpy.fft.ihfft(args[0], **kwargs),
    "Irfft": lambda *args, **kwargs: jax.numpy.fft.irfft(args[0], **kwargs),
    ...
    "Rfft2": lambda *args, **kwargs: jax.numpy.fft.rfft2(args[0], **kwargs),
    "Rfftfreq": lambda *args, **kwargs: jax.numpy.fft.rfftfreq(*args, **kwargs),
...
def execute_op(cls: type, op_type: str, *args: Any, **kwargs: Any) -> Any:
    ...
    schema: Any = load_backend_mappings("jax")
