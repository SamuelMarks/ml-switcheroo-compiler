...
def _np_segment_max(backend_module: object, data: object, segment_ids: object, num_segments: object = None) -> object:
    ...
    out: object = np.full((num_segments,) + data.shape[1:], -np.inf, dtype=data.dtype)
    for i in range(num_segments):
        mask: object = segment_ids == i
        if np.any(mask):
...
def _np_segment_prod(backend_module: object, data: object, segment_ids: object, num_segments: object = None) -> object:
    ...
    if num_segments is None:
        num_segments: object = np.max(segment_ids) + 1
    out: object = np.ones((num_segments,) + data.shape[1:], dtype=data.dtype)
    for i in range(num_segments):
        mask: object = segment_ids == i
        if np.any(mask):
...
def _adaptive_pool_1d_indices(input_dim: int, output_dim: int) -> list[tuple[int, int]]:
    ...
    for i in range(output_dim):
        start: object = int(np.floor(i * input_dim / output_dim))
        end: object = int(np.ceil((i + 1) * input_dim / output_dim))
        start: object = max(0, min(start, input_dim - 1))
        end: object = max(start + 1, min(end, input_dim))
...
def _np_adaptive_avg_pool2d(backend_module: object, operand: object, output_size: tuple[int, int], **kwargs: object) -> object:
    ...
    if isinstance(operand, np.ndarray):
        sh: object = list(operand.shape)
        h_in, w_in = sh[-2], sh[-1]
        h_out, w_out = output_size[0], output_size[1]

        h_indices: object = _adaptive_pool_1d_indices(h_in, h_out)
        w_indices: object = _adaptive_pool_1d_indices(w_in, w_out)

        out_shape: object = sh[:-2] + [h_out, w_out]
        out: object = np.zeros(out_shape, dtype=operand.dtype)

        for i, (h_start, h_end) in enumerate(h_indices):
            for j, (w_start, w_end) in enumerate(w_indices):
                slice_val: object = operand[..., h_start:h_end, w_start:w_end]
...
def _np_adaptive_avg_pool3d(backend_module: object, operand: object, output_size: tuple[int, int, int], **kwargs: object) -> object:
    ...
    if isinstance(operand, np.ndarray):
        ...
        d_indices: object = _adaptive_pool_1d_indices(d_in, d_out)
        h_indices: object = _adaptive_pool_1d_indices(h_in, h_out)
        w_indices: object = _adaptive_pool_1d_indices(w_in, w_out)

        out_shape: object = sh[:-3] + [d_out, h_out, w_out]
        out: object = np.zeros(out_shape, dtype=operand.dtype)
        ...
        for d_idx, (d_start, d_end) in enumerate(d_indices):
            for h_idx, (h_start, h_end) in enumerate(h_indices):
                for w_idx, (w_start, w_end) in enumerate(w_indices):
                    slice_val: object = operand[..., d_start:d_end, h_start:h_end, w_start:w_end]
...
def _np_adaptive_max_pool3d_indices(backend_module: object, operand: object, output_size: tuple[int, int, int], **kwargs: object) -> object:
    ...
    Returns:
            tuple[int, ...]: Result.
    """
    res: object = _np_adaptive_max_pool3d(backend_module, operand, output_size, **kwargs)
