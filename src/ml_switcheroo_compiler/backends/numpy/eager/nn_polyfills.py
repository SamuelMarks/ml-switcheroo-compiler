...
def _np_log_poisson_loss(backend_module, targets, log_input, **kwargs):
...
def _np_all_candidate_sampler(backend_module, true_classes, **kwargs):
...
def _np_ctc_beam_step(beam: dict[tuple[int, ...], tuple[float, float]], log_p, num_classes: int, blank: int, beam_width: int) -> dict[tuple[int, ...], tuple[float, float]]:
...
def _np_ctc_beam_search_decoder(backend_module, inputs, sequence_length, **kwargs):
...
def _np_ctc_unique_labels(backend_module, labels, **kwargs):
...
def _np_normalize_moments(backend_module, counts, mean_ss, variance_ss, shift, **kwargs):
...
def _np_sufficient_statistics(backend_module, x, axes, **kwargs):
...
def _np_weighted_moments(backend_module, x, axes, frequency_weights, **kwargs):
...
def _np_max_pool_with_argmax(backend_module, input, **kwargs):
...
def _np_collapse_repeated(backend_module, labels, **kwargs):
...
def _np_quantized_conv(backend_module, *args, **kwargs):
    ...
    Returns: np.ndarray: The result of the quantized convolution.
    """
    import numpy as np
