...
def argpartition(
    a: object, kth: object, axis: int = -1, kind: str = "introselect", order: object = None
) -> Tensor:
    """Perform an indirect partition along the given axis."""
    if config.eager_mode:
        data = get_active_backend().execute_op(
            "Argpartition", getattr(a, "data", a), kth, axis=axis, kind=kind, order=order
        )
        return Tensor(data, TensorConfig(data.shape, "int64", getattr(a, "device", None)))
...
def partition(
    ...
    if config.eager_mode:
        data = get_active_backend().execute_op(
            "Partition", getattr(a, "data", a), kth, axis=axis, kind=kind, order=order
        )
        return Tensor(
            data,
...
def delete(arr: object, obj: object, axis: int = None) -> Tensor:
    ...
    if config.eager_mode:
        data = get_active_backend().execute_op(
            "Delete", getattr(arr, "data", arr), getattr(obj, "data", obj), axis=axis
        )
        return Tensor(
            data,
            TensorConfig(
                data.shape, getattr(arr, "dtype", "float32"), getattr(arr, "device", None)
            ),
        )
    return _emit_shape_node(
        "Delete", [arr, obj], {"axis": axis}, (None,), getattr(arr, "dtype", "float32")
    )


def diff(
    a: object, n: int = 1, axis: int = -1, prepend: object = None, append: object = None
) -> Tensor:
...
def digitize(x: object, bins: object, right: bool = False) -> Tensor:
    ...
    if config.eager_mode:
        data = get_active_backend().execute_op(
            "Digitize", getattr(x, "data", x), getattr(bins, "data", bins), right=right
        )
        return Tensor(data, TensorConfig(data.shape, "int64", getattr(x, "device", None)))
    return _emit_shape_node(
        "Digitize", [x, bins], {"right": right}, getattr(x, "shape", ()), "int64"
    )
...
def diagonal(a: object, offset: int = 0, axis1: int = 0, axis2: int = 1) -> Tensor:
    ...
    if config.eager_mode:
        data = get_active_backend().execute_op(
            "Diagonal", getattr(a, "data", a), offset=offset, axis1=axis1, axis2=axis2
        )
        return Tensor(
            data,
            TensorConfig(data.shape, getattr(a, "dtype", "float32"), getattr(a, "device", None)),
