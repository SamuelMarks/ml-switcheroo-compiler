...
class WindowHann(OpDef):
    ...
    def infer_shape(self, length: int, **kwargs):
        """Infer shape.
...
class Stft(OpDef):
    ...
    def infer_shape(self, x, nfft: int, noverlap: int = 0, **kwargs):
...
def stft(x: Tensor, nfft: int, noverlap: int = 0):
...
def istft(x: Tensor, nfft: int, noverlap: int = 0):
