...
def _load_rules() -> dict[str, object]:
    ...
    Returns:
        dict[str, object]: The loaded rules.
    """
    yaml_path: object = os.path.join(os.path.dirname(__file__), "rematerialization_rules.yaml")
...
def _estimate_memory(node: IRNode) -> float:
    ...
    if isinstance(shape, (int, float)):
        return 4.0
    bytes_val: object = 4.0
    for dim in shape:
        bytes_val *= dim
    return bytes_val


def _estimate_compute(node: IRNode, rules: dict[str, object]) -> float:
...
def _find_target_nodes(nodes: list[IRNode], consumers: dict[str, list[str]], node_indices: dict[str, int], rules: dict[str, object]) -> list[IRNode]:
    ...
    """
    target_ops: object = set(rules.get("target_ops", []))
    min_mem: object = rules.get("thresholds", {}).get("min_memory_bytes", 1024 * 1024)
    max_ratio: object = rules.get("thresholds", {}).get("max_compute_to_memory_ratio", 10.0)

    to_remat: object = []
    for n in nodes:
        if n.op_type in target_ops:
            mem: object = _estimate_memory(n)
            comp: object = _estimate_compute(n, rules)
            if mem > min_mem and comp / mem < max_ratio and consumers[n.id]:
                max_dist: object = max(node_indices[c] for c in consumers[n.id]) - node_indices[n.id]
                if max_dist > 10:
                    to_remat.append(n)
    return to_remat
...
def rematerialization_pass(graph: IRGraph) -> bool:
    ...
    rules: object = _load_rules()

    nodes: object = DAGTopologicalSorter.sort(graph)
    node_indices: object = {n.id: i for i, n in enumerate(nodes)}
    ...
    to_remat: object = _find_target_nodes(nodes, consumers, node_indices, rules)

    for n in to_remat:
        clone_id: object = f"{n.id}_remat"
        if clone_id not in graph.nodes:
            new_node: object = IRNode(id=clone_id, op_type=n.op_type, inputs=n.inputs.copy())
        ...
        for c_id in consumers[n.id]:
            if node_indices[c_id] - node_indices[n.id] > 10:
                c_node: object = graph.nodes[c_id]
