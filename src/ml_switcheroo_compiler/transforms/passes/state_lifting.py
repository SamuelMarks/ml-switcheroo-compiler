...
def _lift_node_state(node: IRNode, nid: str, block: object) -> bool:
    ...
    if node.op_type == "ReadVariable":
        var_name: object = node.attributes.get("variable_name", f"var_{nid}")
        node.op_type = "Input"
...
def _lift_block(block: object) -> bool:
    ...
    Returns:
        bool: Result.
    """
    block_mod: object = False
    for nid, node in _get_node_items(block):
        block_mod: object = _lift_node_state(node, nid, block) or block_mod

        for attr_val in node.attributes.values():
            if hasattr(attr_val, "nodes"):
                block_mod: object = _lift_block(attr_val) or block_mod
    return block_mod
