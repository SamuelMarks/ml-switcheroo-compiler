...
def _cast_inputs(node: IRNode, graph: IRGraph, target_dtype: str, new_nodes: dict[str, IRNode]) -> tuple[list[str], bool]:
    ...
    for inp_id in node.inputs:
        inp_node: object = new_nodes.get(inp_id) or graph.nodes.get(inp_id)
        current_dtype: object = inp_node.attributes.get("dtype", "float32") if inp_node else "float32"
        if current_dtype != target_dtype:
            # Insert a cast
            cast_id: object = f"{inp_id}_cast_{target_dtype}"
            if cast_id not in new_nodes:
                cast_node: object = IRNode(
                    id=cast_id,
                    op_type="Cast",
                    inputs=[inp_id],
...
def _get_scale_nodes(new_nodes: dict[str, IRNode], scale: float) -> tuple[str, str]:
    ...
    Returns:
            tuple[int, ...]: Result.
    """
    scale_node_id: object = "loss_scale_factor"
    inv_scale_node_id: object = "loss_scale_inv_factor"
    ...
    for g_id in grad_inputs:
        ...
        for n_id, n in list(new_nodes.items()):
            if n_id == mul_id:
                continue
            if g_id in n.inputs:
                new_inputs: object = [mul_id if i == g_id else i for i in n.inputs]
...
def _unscale_outputs(graph: IRGraph, new_nodes: dict[str, IRNode], inv_scale_node_id: str) -> list[str]:
    ...
    for out_id in graph.outputs:
        unscaled_id: object = f"{out_id}_unscaled"
...
def loss_scaling_pass(graph: IRGraph, scale: float = 1024.0) -> bool:
    ...
    new_outputs: object = _unscale_outputs(graph, new_nodes, inv_scale_node_id)
