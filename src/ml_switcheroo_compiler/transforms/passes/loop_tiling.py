...
def _load_heuristics() -> dict[str, Any]:
    ...
    Returns:
        dict[str, Any]: The loaded heuristics.
    """
...
def _get_tiling_config() -> dict[str, Any]:
    """Get the tiling configuration based on active backend.

    Returns:
        dict[str, Any]: The tiling configuration.
    """
...
def _should_tile(op_type: str, shape: Any, op_config: dict[str, Any]) -> bool:
    ...
    Args:
        ...
        shape (Any): The node's shape metadata.
        op_config (dict[str, Any]): The operation's configuration.
